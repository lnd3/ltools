#include "serialization/BaseAscii.h"
...
namespace l::serialization {
	...
	std::string baseascii_encode(unsigned char* src, size_t len, bool tolowercase) {
		std::string out;
	...
	std::string baseascii_encode(std::string_view src, bool tolowercase) {
		std::string out;
		const char* svec = tolowercase ? s_vecLower : s_vecUpper;
	...
	std::string baseascii_decode(std::string_view src) {
		std::string out;
		auto size = src.size();
		size = size - size % 2;
		for (size_t i = 0; i < size; i += 2) {
