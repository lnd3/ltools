...
namespace l::rendering {
	namespace image {
		...
		image_u8& image_u8::crop(uint32_t new_width, uint32_t new_height) {
			...
			const uint32_t w = math::min2(m_width, new_width);
			const uint32_t h = math::min2(m_height, new_height);
		...
		void image_metrics::compute(const image_u8& a, const image_u8& b, uint32_t first_channel, uint32_t num_channels) {
			...
			const uint32_t width = math::min2(a.width(), b.width());
			const uint32_t height = math::min2(a.height(), b.height());
